...
static int msm_fb_blank_sub(int blank_mode, struct fb_info *info,
                           boolean op_enable);
static int msm_fb_blank_sub0(int blank_mode, struct fb_info *info,
			    boolean op_enable);
                           ...
                           boolean op_enable)
{
       ...
       down(&msm_fb_pan_sem);
       ret = msm_fb_blank_sub0(blank_mode, info, op_enable);
       up(&msm_fb_pan_sem);
			      ...
			      struct fb_info *info)
{
	...
	if (info->node == 0 && !(mfd->cont_splash_done)) { /* primary */
		mdp_set_dma_pan_info(info, NULL, TRUE);
		if (msm_fb_blank_sub0(FB_BLANK_UNBLANK, info, mfd->op_enable)) {
			up(&msm_fb_pan_sem);
