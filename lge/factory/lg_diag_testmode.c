...
//    if(gpio_get_value(PM8058_GPIO_PM_TO_SYS(PMIC_GPIO_SDC3_DET - 1)))
    ...
    switch( pReq->esm){
        ...
        if (sys_statfs(EXTERNALSD_MOUNT_POINT, (struct statfs *)&sf) != 0)
            ...
            pRsp->test_mode_rsp.socket_memory_usedsize = (((long long)sf.f_blocks - (long long)sf.f_bfree)* (long long)sf.f_bsize);

            if(pRsp->test_mode_rsp.socket_memory_usedsize <= 425984)//~416kb default folder
            ...
            {
                pRsp->ret_stat_code = TEST_FAIL_S;
                printk(KERN_ERR "[CALCUSEDSIZE] DiagCommandObserver returned fail or didn't return in 1000ms.\n");
            }
#endif
            break;
	...
	switch(req_ptr.test_mode_req.sleep_mode){	
		...
		case SLEEP_MODE_ON:
			...
			if(LGE_BOOT_MODE_MINIOS == get_lge_boot_mode())
			{
				...
				if (diagpdev != NULL){
					update_diagcmd_state(diagpdev, "FLIGHT_SLEEP_ON", 0);
				}
				else
					pRsp->ret_stat_code = TEST_FAIL_S;
			}
		...
		case FLIGHT_MODE_OFF:
			...
			if(LGE_BOOT_MODE_MINIOS == get_lge_boot_mode())
			{
				...
				if (diagpdev != NULL){
					update_diagcmd_state(diagpdev, "FLIGHT_SLEEP_OFF", 0);
				}
				else
					pRsp->ret_stat_code = TEST_FAIL_S;
			}
...
// LGE_CHANGE_S, seongjin.noh@lge.com, 2013-06-22, DIAG Emergency Call Test
    {TEST_MODE_EMERGENCY_CALL, NULL, ARM9_PROCESSOR},
    /*{TEST_MODE_LTE_MODE_SELECTION, not_supported_command_handler, ARM11_PROCESSOR},*/
// LGE_CHANGE_E, seongjin.noh@lge.com, 2013-06-22, DIAG Emergency Call Test
